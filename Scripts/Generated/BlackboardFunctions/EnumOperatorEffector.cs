...
{
    public unsafe partial class EnumOperatorEffector : HiraBotsEffectorBlackboardFunction
    {
        private struct Memory
        {
            internal BlackboardKey.LowLevel _key;
            internal HiraBots.EnumOperationType _operationType;
            internal byte _value;
        }

        [SerializeField] internal BlackboardTemplate.KeySelector key;
        [SerializeField] internal HiraBots.EnumOperationType operationType;
        ...
        private static void ActualFunction(in BlackboardComponent.LowLevel blackboard, byte* rawMemory)
        {
            ...
            HiraBots.SampleEffectorBlackboardFunctions.EnumOperatorEffector(ref blackboard.Access<byte>(memory->_key.offset), memory->_operationType, memory->_value);
        }

        // non-VM execution
        protected override void ExecuteFunction(BlackboardComponent blackboard, bool expected)
        {
            var _key = blackboard.GetEnumValue(key.selectedKey.name); HiraBots.SampleEffectorBlackboardFunctions.EnumOperatorEffector(ref _key, operationType, value); blackboard.SetEnumValue(key.selectedKey.name, _key, expected);
        }
